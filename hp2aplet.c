...
char *  addDataPath(char * fileName){
	...
	if(dataFolder[1]==':')strcat(slack,"\\bin\\"); //we're on windows
	else strcat(slack,"/bin/"); //we're on *nix
...
void writeFiveBytes(LONGLONG bytes, unsigned char * buffer, unsigned int offset){
...
void writeSixBytes(LONGLONG bytes, unsigned char * buffer, unsigned int offset){
...
////////////////////////////	
	...
	writeFiveBytes(0x2a2c+(LONGLONG) ((strlen(apletName)*2+5)<<20),aplet, position); 
	position+=5;
	
	strcpy((char *)aplet+position,apletName);
	position+=strlen(apletName); //length of the title

	writeFiveBytes(0x2a2c+(LONGLONG)((strlen(authorName)*2+5)<<20),aplet, position); 
	position+=5;
...
///////////////////////////
	...
	writeFiveBytes(0x2911+(LONGLONG)(X<<20),aplet,position); //write grob position
	position+=5;
	writeFiveBytes(0x2911+(LONGLONG)(Y<<20),aplet,position);
	position+=5;
...
////////////////////////////////////////
	...
	writeFiveBytes((LONGLONG)0x2a74 + (((LONGLONG)0x312b) << 20),aplet,position);
	position+=5;
...
///////////////////////////
	...
	LONGLONG constant = (LONGLONG)0x312b + (((LONGLONG)vf_offset)<<20);
